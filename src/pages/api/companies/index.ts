...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    const body = { ...req.body };
    if (body?.calculation_formula?.length > 0) {
      const create_calculation_formula = body.calculation_formula;
      body.calculation_formula = {
        create: create_calculation_formula,
      };
    } else {
      delete body.calculation_formula;
    }
